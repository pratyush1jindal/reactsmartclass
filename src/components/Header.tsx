...
export default function Header() {
  ...
  const userName = useAppSelector((smartApp) => smartApp.auth.userInfo?.name);
  const isDarkTheme = useAppSelector((smartApp) => smartApp.auth.isDarkTheme);
  ...
  const invertTheme = () => {
    const theme = localStorage.getItem("smart-theme");
    localStorage.setItem("smart-theme", theme === "light" ? "dark" : "light");
        ...
        <Link to="/">
          <EuiText>
            <h2 style={{ padding: "0 1vw" }}>
              <EuiTextColor color="#0b5cff">SMART CLASSROOM</EuiTextColor>
            </h2>
          </EuiText>
        </Link>,
      ],
